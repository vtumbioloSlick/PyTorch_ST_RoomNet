...
# PyTorch Affine Transformer
class AffineTransformer(torch.nn.Module):
    def __init__(self, out_size):
        super().__init__()
        self.out_size = out_size  # (height, width)

    def forward(self, input_tensor, theta):
        theta = theta.view(-1, 2, 3)
        B, C, H, W = input_tensor.shape
        grid = F.affine_grid(theta, (B, C, *self.out_size), align_corners=True)
        return F.grid_sample(input_tensor, grid, mode='bilinear', align_corners=True)

# PyTorch Projective Transfomer
class ProjectiveTransformer(torch.nn.Module):
    def __init__(self, out_size, interp_method='bilinear'):
        super().__init__()
        self.out_size = out_size  # (height, width)
        self.interp_method = interp_method
        self.pixel_grid = self._meshgrid(out_size)  # [3, H*W]

    def forward(self, inp, theta):
        """
        inp: [B, C, H, W]
        theta: [B, 8]
        """
        B, C, H, W = inp.shape
        x_s, y_s = self._transform(theta, B)

        output = self._interpolate(inp, x_s, y_s, self.out_size, method=self.interp_method)

        output = output.view(B, C, self.out_size[0], self.out_size[1])

        return output

    def _transform(self, theta, batch_size):
        theta = theta.view(batch_size, 8)
        theta = torch.cat([theta, torch.ones((batch_size, 1), device=theta.device)], dim=1)  # Now [B, 9]
        theta = theta.view(batch_size, 3, 3)

        pixel_grid = self.pixel_grid.unsqueeze(0).repeat(batch_size, 1, 1)  # [B, 3, H*W]

        T_g = torch.bmm(theta, pixel_grid)  # [B, 3, H*W]

        x_s = T_g[:, 0, :]
        y_s = T_g[:, 1, :]
        z_s = T_g[:, 2, :] + 1e-7  # prevent division by zero
    ...
    def _meshgrid(self, out_size):
        height, width = out_size
        x_t = torch.linspace(-1.0, 1.0, width)
        y_t = torch.linspace(-1.0, 1.0, height)
        x_t, y_t = torch.meshgrid(y_t, x_t, indexing='ij')  # Careful: (H, W)
        x_t = x_t.contiguous().view(1, -1)
        y_t = y_t.contiguous().view(1, -1)
        ones = torch.ones_like(x_t)
        grid = torch.cat([x_t, y_t, ones], dim=0)  # [3, H*W]
        return grid

    def _interpolate(self, im, x, y, out_size, method='bilinear'):
        """
        im: [B, C, H, W]
        x, y: [B, H*W] in normalized coordinates (-1 to 1)
        """
        B, C, H, W = im.shape
        H_out, W_out = out_size

        # Stack and normalize to [-1, 1]
        x = x.view(B, H_out, W_out)
        y = y.view(B, H_out, W_out)

        grid = torch.stack((y, x), dim=-1)  # [B, H_out, W_out, 2]
        grid = grid.clamp(-2, 2)  # Hard clipping for safety

        out = F.grid_sample(im, grid, mode=method, align_corners=False)
    ...
    def _transform(self, theta, batch_size):
        theta = theta.view(batch_size, 8)
        theta = torch.cat([theta, torch.ones((batch_size, 1), device=theta.device)], dim=1)  # [B, 9]
        theta = theta.view(batch_size, 3, 3)

        pixel_grid = self.pixel_grid.to(theta.device).unsqueeze(0).repeat(batch_size, 1, 1)  # [B, 3, H*W]

        T_g = torch.bmm(theta, pixel_grid)  # [B, 3, H*W]

        x_s = T_g[:, 0, :]
        y_s = T_g[:, 1, :]
        z_s = T_g[:, 2, :] + 1e-7  # avoid division by zero
